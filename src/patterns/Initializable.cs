...
namespace MaxiNet
{
    ...
    public abstract class Initializable : IInitializable
    {
        ...
        public Result<Nothing> Initialize()
        {
            ...
            if (_isInitializing)
            {
                return new NegativeResult<Nothing>(new Oration("Initialization is already in progress on ?", [GetType().Name]));
            }
            ...
            catch (Exception ex)
            {
                ...
                return new ExceptionResult<Nothing>(ex, new Oration("An exception occurred during initialization: ?", [ex.Message]));
            }
